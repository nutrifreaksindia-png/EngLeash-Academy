...
router.get('/', auth, requireRole('Admin', 'Trainer', 'Creator'), (req, res) => {
  let rows = db.prepare(`
    ...
    FROM worksheet_library m
    LEFT JOIN users cu ON cu.id = m.created_by
    LEFT JOIN worksheet_assignments a ON a.worksheet_id = m.id
    LEFT JOIN worksheet_assets s ON s.worksheet_id = m.id
    GROUP BY m.id
...
router.get('/:id', auth, (req, res) => {
  ...
  const row = db.prepare(
    'SELECT m.*, cu.name AS creator_name FROM worksheet_library m LEFT JOIN users cu ON cu.id = m.created_by WHERE m.id = ?'
  ).get(id);
  if (!row) return res.status(404).json({ error: 'Worksheet not found' });
  if (!canViewWorksheet(req.user, row)) {
...
router.put('/:id', auth, requireRole('Admin', 'Trainer', 'Creator'), (req, res) => {
  const id = Number(req.params.id);
  const row = db.prepare('SELECT * FROM worksheet_library WHERE id = ?').get(id);
  if (!row) return res.status(404).json({ error: 'Worksheet not found' });
...
router.delete('/:id', auth, requireRole('Admin', 'Trainer', 'Creator'), async (req, res) => {
  ...
  if (isSpacesConfigured()) {
    try {
      await deleteObjectsUnderPrefix(`pre-recorded/worksheets/worksheet-${id}/`);
    } catch (e) {
      console.error('[worksheet-delete] Spaces', id, e);
    }
  }
  db.prepare('DELETE FROM worksheet_library WHERE id = ?').run(id);
  res.status(204).end();
});
...
router.post('/:id/assets', auth, requireRole('Admin', 'Trainer', 'Creator'), uploadSingle, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const worksheet = db.prepare('SELECT * FROM worksheet_library WHERE id = ?').get(id);
    if (!worksheet) return res.status(404).json({ error: 'Worksheet not found' });
    if (!canMutateLibraryByCreatedBy(req.user, worksheet)) return res.status(403).json({ error: 'Forbidden' });
    ...
    const key = `pre-recorded/worksheets/worksheet-${id}/${assetType}/${Date.now()}-${base}.${ext}`;
    ...
    const r = db.prepare(`
      INSERT INTO worksheet_assets (worksheet_id, asset_type, url, storage_key, meta_json, sort_order)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(id, assetType, uploaded.publicUrl, uploaded.key, metaJson, Number.isFinite(sortOrder) ? sortOrder : 0);
    res.status(201).json(db.prepare('SELECT * FROM worksheet_assets WHERE id = ?').get(r.lastInsertRowid));
  } catch (error) {
...
router.delete('/:id/assets/:assetId', auth, requireRole('Admin', 'Trainer', 'Creator'), (req, res) => {
  ...
  if (!worksheet) return res.status(404).json({ error: 'Worksheet not found' });
  if (!canMutateLibraryByCreatedBy(req.user, worksheet)) return res.status(403).json({ error: 'Forbidden' });
  db.prepare('DELETE FROM worksheet_assets WHERE id = ? AND worksheet_id = ?').run(assetId, id);
  res.status(204).end();
});
...
router.post('/:id/assignments', auth, requireRole('Admin', 'Trainer', 'Creator'), (req, res) => {
  ...
  res.status(201).json(db.prepare('SELECT * FROM worksheet_assignments WHERE id = ?').get(r.lastInsertRowid));
});
...
router.get('/:id/assignments', auth, requireRole('Admin', 'Trainer', 'Creator'), (req, res) => {
  ...
  const worksheet = db.prepare('SELECT * FROM worksheet_library WHERE id = ?').get(id);
  if (!worksheet) return res.status(404).json({ error: 'Worksheet not found' });
  if (!canViewWorksheet(req.user, worksheet)) return res.status(404).json({ error: 'Worksheet not found' });
  const rows = db.prepare('SELECT * FROM worksheet_assignments WHERE worksheet_id = ? ORDER BY created_at DESC').all(id);
  res.json(rows);
});
...
router.delete('/assignments/:assignmentId', auth, requireRole('Admin', 'Trainer', 'Creator'), (req, res) => {
  ...
  const ass = db.prepare(`
    ...
    JOIN worksheet_library m ON m.id = a.worksheet_id
    WHERE a.id = ?
  `).get(assignmentId);
  ...
  db.prepare('DELETE FROM worksheet_assignments WHERE id = ?').run(assignmentId);
  res.status(204).end();
});
