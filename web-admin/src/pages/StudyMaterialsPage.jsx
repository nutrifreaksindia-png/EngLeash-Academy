...
export default function StudyMaterialsPage({
  ...
  async function saveDraft() {
    ...
    try {
      ...
      setAutosaveStatus('saved');
      window.setTimeout(() => setDraftNotice(''), 5000);
      if (autosaveClearSavedTimerRef.current) window.clearTimeout(autosaveClearSavedTimerRef.current);
      autosaveClearSavedTimerRef.current = window.setTimeout(() => {
        setAutosaveStatus((s) => (s === 'saved' ? 'idle' : s));
        autosaveClearSavedTimerRef.current = null;
  ...
  function applyPasteNormalization() {
    ...
    setDoc((prev) => ({ ...prev, blocks: [...prev.blocks, ...nextBlocks] }));
                      ...
                      <div className="row studyMaterialRowActions">
                        ...
                        {libraryCanMutate(m) ? (
                          <>
                            ...
                            <button type="button" className="dangerBtn" onClick={() => onDelete(m.id)}>
                              Delete
                            </button>
                          </>
                        ) : null}
          ...
          <form className="studyFullscreenShell" onSubmit={submit}>
            <div className="studyFullscreenTopbar">
              ...
              <div className="studyTopActions">
                {['heading', 'paragraph', 'highlightedQuote', 'bulletList', 'numberedList', 'image', 'imageCarousel', 'gif', 'table', 'pdfAttachment', 'audioAttachment', 'divider'].map((t) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setDoc((prev) => ({ ...prev, blocks: [...prev.blocks, created] }));
                ...
                <button type="button" className="secondaryBtn" disabled={creatingDraft || !editingId} onClick={() => saveDraft()}>
                  Save draft
                </button>
                <button type="submit" className="uploadPrimaryBtn" disabled={creatingDraft || !editingId}>
                  {editingId ? 'Publish changes' : 'Publish'}
                </button>
                <button type="button" className="dangerBtn" onClick={resetEditor}>Close</button>
              </div>
                    ...
                    >
                      <div className="studyBlockHead">
                        ...
                        <div className="row">
                          ...
                          <button type="button" className="dangerBtn" onClick={() => setDoc((prev) => ({ ...prev, blocks: prev.blocks.filter((x) => x.id !== b.id) }))}>Remove</button>
                        </div>
                      </div>
